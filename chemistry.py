...
#I had to make all of this myself becuase the test wouldn't accept my dictionary I made from this list. 
...
#My first attempt at making a dictionary from the previous list. This was a success, but it didn't work well with the test. 
def make_periodic_table2(periodic_table):
    symbols = []
    names_and_atomic_mass = []

    for elements in periodic_table:
        symbols.append(elements[0])
        names_and_atomic_mass.append([elements[1], elements[2]])
    
    periodic_dictionary = dict(zip(symbols, names_and_atomic_mass))

    return periodic_dictionary



#Returns the name for whatever element is given
def get_name(periodic_table, element):
    name_index = 1
    
    for elements in periodic_table:
        if element.capitalize() in elements[0]:
            return elements[1]
        else:
            bob = 'bob'


#Returns the atomic mass for whatever element is given
def get_atomic_mass(periodic_table, element):
    ...
    def parse_r(formula, index, level):
        ...
        while index < len(formula):
            ...
            if ch == "(":
                ...
                symbol = symbol.capitalize()
                if symbol.capitalize() in periodic_table:
                    index += 2
                else:
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for element in symbol_quantity_list:
        ...
        value = periodic_table[symbol.capitalize()]
        atomic_mass = value[ATOMIC_MASS_INDEX]

        molecule_mass = atomic_mass * quantity
        total_mass = total_mass + molecule_mass
...
def main():
    #prints the list to look nice
    table = make_periodic_table_1()
    for elements in table:
        print(elements)


    #makes my dictionary from the list
    periodic_dictionary = make_periodic_table2(table)



    

    #Asks for molecular formula and mass so I can use that info and print the moler mass later on
    molecular_formula = input('Please enter a molecular formula of the sample: ')
    mass = float(input('Enter the mass in grams of the sample: '))

    computable_formula = parse_formula(molecular_formula, periodic_dictionary)

    molar_mass = compute_molar_mass(computable_formula, periodic_dictionary)

    print(f'{molar_mass:.5f} grams/mole')

    total_moles = mass/molar_mass

    print(f'{total_moles:.5f} moles')

    element_symbol = input("Please type the element symbol: ")

    #print element name
    name = get_name(table, element_symbol)
    print()
    print(name)
    print()

    #print atomic mass
    atomic_mass = get_atomic_mass(table, element_symbol)
    print(atomic_mass)
    print()
